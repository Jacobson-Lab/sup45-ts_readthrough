...
p_all <- f6b_plot(df = df_all, x_axis_label = "nt +4")
p_High <- f6b_plot(df = df_High, x_axis_label = "nt +4")
p_Low <- f6b_plot(df = df_Low, x_axis_label = "nt +4")

# Arrange plots
f6b <- ggarrange(plotlist = lapply(list(p_all, p_High, p_Low), "+", coord_equal()), 
                 common.legend = TRUE, nrow = 1, 
                 labels = c("All", "High", "Low"), label.x = 0.66, hjust = 0.5, font.label = list(size = 8, face = "plain"))

# Save plot
ggsave(filename = "Figure6B.pdf", plot = f6b, path = "../Plots", height = 2.5, width = 7, units = "in", dpi = 500)
